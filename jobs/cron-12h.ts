...
export function cron12hJob() {
  // ✅ Schedule cron job
  Deno.cron("Handle 12h task at 3-AM UTC", "0 4 * * *", runTask);
  Deno.cron("Handle 12h task at 3-PM UTC", "0 16 * * *", runTask);
}

async function runTask() {
  const currentTime = UnixToTime(new Date().getTime());
  logger.info(`⏳ Cron 12h Job ${currentTime} is running...`, DColors.cyan);
  const timeframe = TF.h12;
